    ...
    var JSONCompletion = (function () {
            ...
            return this.schemaService.getSchemaForResource(document.uri, doc).then(function (schema) {
                ...
                if (node && node.type === 'object') {
                    ...
                    if ((!schema && currentWord.length > 0 && document.getText().charAt(offset - currentWord.length - 1) !== '"')) {
                        collector.add({ kind: 10 /* Property */, label: _this.getLabelForValue(currentWord), insertText: _this.getInsertTextForProperty(currentWord, null, false, separatorAfter_1), documentation: '' });
                    }
                }
        ...
        JSONCompletion.prototype.getSchemaLessPropertyCompletions = function (doc, node, currentKey, collector) {
            ...
            if (node.parent) {
                ...
                collector.add({ kind: 10 /* Property */, label: '$schema', insertText: this.getInsertTextForProperty('$schema', null, true, ''), documentation: '', filterText: this.getFilterTextForValue("$schema") });
            }
        };
        JSONCompletion.prototype.getSchemaLessValueCompletions = function (doc, node, offset, separatorAfter, document, collector) {
            ...
            var collectSuggestionsForValues = function (value) {
                if (!value.parent.contains(offset, true)) {
                    collector.add({ kind: _this.getSuggestionKind(value.type), label: _this.getLabelTextForMatchingNode(value, document), insertText: _this.getInsertTextForMatchingNode(value, document, separatorAfter), documentation: '' });
                }
            ...
            if (Array.isArray(schema.defaultSnippets)) {
                schema.defaultSnippets.forEach(function (s) {
                    ...
                    if (value) {
                        ...
                        insertText = vscode_languageserver_types_1.SnippetString.create(prefix + indent + s.bodyText.split('\n').join('\n' + indent) + suffix + separatorAfter);
        ...
        JSONCompletion.prototype.addDollarSchemaCompletions = function (separatorAfter, collector) {
            ...
            schemaIds.forEach(function (schemaId) { return collector.add({ kind: 9 /* Module */, label: _this.getLabelForValue(schemaId), filterText: JSON.stringify(schemaId), insertText: _this.getInsertTextForValue(schemaId, separatorAfter), documentation: '' }); });
        };
        ...
        JSONCompletion.prototype.getInsertTextForPlainText = function (text) {
            return vscode_languageserver_types_1.SnippetString.create(text.replace(/[\\\$\}]/g, '\\$&')); // escape $, \ and } 
        };
        JSONCompletion.prototype.getInsertTextForValue = function (value, separatorAfter) {
            var text = JSON.stringify(value, null, '\t');
            if (text === '{}') {
                return vscode_languageserver_types_1.SnippetString.create('{\n\t$1\n}' + separatorAfter);
            }
            else if (text === '[]') {
                return vscode_languageserver_types_1.SnippetString.create('[\n\t$1\n]' + separatorAfter);
            }
        ...
        JSONCompletion.prototype.getInsertTextForGuessedValue = function (value, separatorAfter) {
            switch (typeof value) {
                case 'object':
                    if (value === null) {
                        return vscode_languageserver_types_1.SnippetString.create('${1:null}' + separatorAfter);
                    }
                ...
                case 'string':
                    ...
                    return vscode_languageserver_types_1.SnippetString.create('"${1:' + snippetValue + '}"' + separatorAfter);
                case 'number':
                case 'integer':
                case 'boolean':
                    return vscode_languageserver_types_1.SnippetString.create('${1:' + JSON.stringify(value) + '}' + separatorAfter);
            }
            return this.getInsertTextForValue(value, separatorAfter);
        };
