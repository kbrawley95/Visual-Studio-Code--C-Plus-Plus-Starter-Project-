...
class TextDocuments {
    ...
    constructor() {
        ...
        this._onWillSave = new vscode_jsonrpc_1.Emitter();
    }
    /**
...
class BulkUnregistrationImpl {
    constructor(connection, console, _unregistrations) {
        this.connection = connection;
        this.console = console;
        this._unregistrations = _unregistrations;
    }
    dispose() {
        let params = {
            unregisterations: this._unregistrations
        };
        this.connection.sendRequest(protocol_1.UnregistrationRequest.type, params).then(undefined, (error) => {
    ...
    disposeSingle(index) {
        ...
        this.connection.sendRequest(protocol_1.UnregistrationRequest.type, params).then(() => {
...
class RemoteClientImpl {
    ...
    register(typeOrRegistrations, registerOptions) {
        if (typeOrRegistrations instanceof BulkRegistrationImpl) {
            return this.registerMany(typeOrRegistrations);
        }
        else {
            return this.registerSingle(typeOrRegistrations, registerOptions);
        }
    }
    registerSingle(type, registerOptions) {
        const method = Is.string(type) ? type : type.method;
        const id = UUID.generateUuid();
        let params = {
            registrations: [{ id, method, registerOptions }]
        };
        return this.connection.sendRequest(protocol_1.RegistrationRequest.type, params).then((result) => {
    ...
    unregisterSingle(id, method) {
        ...
        return this.connection.sendRequest(protocol_1.UnregistrationRequest.type, params).then(undefined, (error) => {
            this.console.info(`Unregistering request handler for ${id} failed.`);
            throw error;
        });
    }
    registerMany(registractions) {
        let params = registractions.asRegistrationParams();
        return this.connection.sendRequest(protocol_1.RegistrationRequest.type, params).then(() => {
...
function createConnection(input, output) {
    ...
    let protocolConnection = {
        ...
        onInitialized: (handler) => connection.onNotification(protocol_1.InitializedNotification.type, handler),
        ...
        onWillSaveTextDocument: (handler) => connection.onNotification(protocol_1.WillSaveTextDocumentNotification.type, handler),
        onWillSaveTextDocumentWaitUntil: (handler) => connection.onRequest(protocol_1.WillSaveTextDocumentWaitUntilRequest.type, handler),
    ...
    connection.onRequest(protocol_1.ShutdownRequest.type, () => {
        shutdownReceived = true;
        if (shutdownHandler) {
            return shutdownHandler(new vscode_jsonrpc_1.CancellationTokenSource().token);
        }
        else {
            return undefined;
        }
    });
    connection.onNotification(protocol_1.ExitNotification.type, () => {
        try {
            if (exitHandler) {
                exitHandler();
            }
